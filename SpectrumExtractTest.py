...
# Reading the data and setting up the initial arrays
path = "../J0156/"
dataFile = path+"ADP.2021-10-13T20:49:23.745.fits"
lines = pd.read_csv("../Lines.txt", sep = r"\s+")
def fit(dataFile, xPix, yPix, medwdw, linefile, stHa, zg, wg):
	...
	# Minimizing residual
	result = minimize(gaussFit, pars, args=(df["wl"].values,), kws={"f":df["maskFlux"].values, "lines":lines}, nan_policy="omit")
	...
	return result, wl, flux, fluxMask


# Plotting the fit
plt.plot(wl, fluxMask, linewidth=0.8)
plt.plot(wl, gaussFit(result.params, wl, lines=linefile["wl"], 
